\begin{filecontents*}{template/references.bib}
...
\documentclass{template/stageOnera}
%%
% Paquets
%
...
J'ai fais mon stage dans le service MIDL du DTIS. Le sujet est interdisciplinaire : il croise linguistique de corpus, psycholinguistique des émotions et apprentissage automatique appliqué à la détection de contenus abusifs (\textit{Abusive Language Detection}).
...
J'ai par ailleurs annoté manuellement 781~messages issus de \textit{CyberAgression-Large} pour produire le gold émotionnel \textit{CyberAggAdo}, compatible avec les sorties d'EMOTYC.\footnote{Gold et visualisations : \href{https://github.com/GwenTsang/Eval-EMOTYC/blob/main/golds/CyberAdoAgg_gold_global_total.xlsx}{Visualisation des annotations}, \href{https://emotyc.github.io/}{Visualisations corpus CyberAggAdo}, \href{https://gwentsang.github.io/}{Visualisation corpus TextToKids}.}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth,height=0.25\textheight,keepaspectratio]{figures/schemas/schema.pdf}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrccc}
    ...
    \bottomrule
  \end{tabular}
  \caption{Heatmap des écarts de performance entre \textit{TextToKids} et \textit{CyberAggAdo}.}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrccc}
    ...
    \bottomrule
  \end{tabular}
  \caption{Labels concentrant le plus fort décrochage entre \textit{TextToKids} et \textit{CyberAggAdo}.}
  \label{tab:heatmap-decrochage}
\end{table}

On voit que le mode \textit{Suggérée} et le label \textit{Autre} présentent les écarts les plus marqués.
...
%% Bibliographie
\bibliographystyle{template/onera}
